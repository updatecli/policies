...
Automatically update the Hugo version referenced in `netlify.toml` to the latest Hugo release.
...
- updates the `HUGO_VERSION = "..."` field in `netlify.toml`
...
- a `netlify.toml` file in the repository with a `HUGO_VERSION` variable
...
The following example updates the Hugo version in `netlify.toml` and opens a pull request:
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/hugo/netlify:0.10.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/hugo/netlify:0.10.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/hugo/netlify:0.10.0
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/hugo/netlify:0.10.0"
```
...
2. Confirm that `netlify.toml` exists and contains a `HUGO_VERSION` variable under `[build.environment]`.
