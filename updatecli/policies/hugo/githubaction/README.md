# Hugo GitHub Action Policy

Automatically update the Hugo version referenced in GitHub Actions workflow files to the latest Hugo release.

## Overview

This policy runs a single Updatecli pipeline that:

- fetches the latest `gohugoio/hugo` release from GitHub releases
- updates the `hugo-version` field in all `.github/workflows/*.yaml` and `.github/workflows/*.yml` files
- strips the leading `v` prefix from the version before writing (e.g. `v0.136.0` becomes `0.136.0`)

This pattern is commonly used with the `peaceiris/actions-hugo` GitHub Action, which expects the version without the `v` prefix.
...
The following example updates Hugo in GitHub Action workflows and opens a pull request:
...
The `github` block provides credentials for the `githubrelease` source plugin. The version written to workflow files has its leading `v` stripped. If `scm.enabled` is `true`, the policy injects a `default` SCM and action to open pull requests.
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/hugo/githubaction:0.11.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/hugo/githubaction:0.11.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/hugo/githubaction:0.11.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/hugo/githubaction" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/hugo/githubaction:0.11.0"
```
...
2. Confirm that your workflow files contain a `hugo-version:` field (typically inside a `peaceiris/actions-hugo` step).
...
- peaceiris/actions-hugo repository: <https://github.com/peaceiris/actions-hugo>
