# Dockerfile Autodiscovery Policy

This policy wraps Updatecli autodiscovery for Dockerfile projects.
It scans repositories for `Dockerfile` files, generates Updatecli manifests for detected dependencies, and can optionally open pull requests through a supported SCM provider.
...
- detect base image updates in Dockerfiles
...
The policy does not hardcode dependency rules. Instead, it passes your `spec` values directly to the Dockerfile autodiscovery crawler so you can tune behavior with the same options supported by the Updatecli Dockerfile autodiscovery plugin.
...
- `spec`: raw Dockerfile autodiscovery crawler configuration

### Example Values

The following example updates Dockerfile base image dependencies with patch releases only and opens one pull request per detected update:

```yaml
name: "deps(dockerfile): bump patch dependencies"
pipelineid: "dockerfile/autodiscovery/patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/dockerfile:0.9.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/dockerfile:0.9.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/dockerfile:0.9.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/dockerfile" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/dockerfile:0.9.0"
```
...
1. Show the rendered manifest and verify that the `dockerfile` crawler is present:
...
- Dockerfile autodiscovery plugin docs: <https://www.updatecli.io/docs/plugins/autodiscovery/dockerfile/>
