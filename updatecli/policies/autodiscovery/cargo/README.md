# Cargo Autodiscovery Policy

This policy wraps Updatecli autodiscovery for Cargo projects.
It scans repositories for `Cargo.toml` files, generates Updatecli manifests for detected dependencies, and can optionally open pull requests through a supported SCM provider.
...
- detect Rust crate dependency updates
...
The policy does not hardcode dependency rules. Instead, it passes your `spec` values directly to the Cargo autodiscovery crawler so you can tune behavior with the same options supported by the Updatecli Cargo autodiscovery plugin.
...
- `spec`: raw Cargo autodiscovery crawler configuration

### Example Values

The following example updates cargo crate dependencies with patch releases only and opens one pull request per detected update:

```yaml
name: "deps(cargo): bump patch dependencies"
pipelineid: "cargo/autodiscovery/patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/cargo:0.9.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/cargo:0.9.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/cargo:0.9.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/cargo" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/cargo:0.9.0"
```
...
1. Show the rendered manifest and verify that the `cargo` crawler is present:
...
- Cargo autodiscovery plugin docs: <https://www.updatecli.io/docs/plugins/autodiscovery/cargo/>
