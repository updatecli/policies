# Ko Autodiscovery Policy

This policy wraps Updatecli autodiscovery for Ko projects.
It scans repositories for Ko configuration files, generates Updatecli manifests for detected dependencies, and can optionally open pull requests through a supported SCM provider.
...
- detect container image updates in Ko build configurations
...
The policy does not hardcode dependency rules. Instead, it passes your `spec` values directly to the Ko autodiscovery crawler so you can tune behavior with the same options supported by the Updatecli Ko autodiscovery plugin.
...
- `spec`: raw Ko autodiscovery crawler configuration

### Example Values

The following example updates Ko container image dependencies with patch releases only and opens one pull request per detected update:

```yaml
name: "deps(ko): bump patch dependencies"
pipelineid: "ko/autodiscovery/patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/ko:0.9.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/ko:0.9.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/ko:0.9.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/ko" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/ko:0.9.0"
```
...
1. Show the rendered manifest and verify that the `ko` crawler is present:
...
- Ko autodiscovery plugin docs: <https://www.updatecli.io/docs/plugins/autodiscovery/ko/>
