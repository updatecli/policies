# Docker Compose Autodiscovery Policy

This policy wraps Updatecli autodiscovery for Docker Compose projects.
It scans repositories for `docker-compose.yml` files, generates Updatecli manifests for detected dependencies, and can optionally open pull requests through a supported SCM provider.
...
- detect Docker image version updates in Docker Compose files
...
The policy does not hardcode dependency rules. Instead, it passes your `spec` values directly to the Docker Compose autodiscovery crawler so you can tune behavior with the same options supported by the Updatecli Docker Compose autodiscovery plugin.
...
- `spec`: raw Docker Compose autodiscovery crawler configuration

### Example Values

The following example updates Docker Compose dependencies with patch releases only and opens one pull request per detected update:

```yaml
name: "deps(dockercompose): bump patch dependencies"
pipelineid: "dockercompose/autodiscovery/patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/dockercompose:0.9.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/dockercompose:0.9.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/dockercompose:0.9.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/dockercompose" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/dockercompose:0.9.0"
```
...
1. Show the rendered manifest and verify that the `dockercompose` crawler is present:
...
- Docker Compose autodiscovery plugin docs: <https://www.updatecli.io/docs/plugins/autodiscovery/dockercompose/>
