...
This policy wraps Updatecli autodiscovery for Updatecli configuration files.
It scans repositories for Updatecli manifests and compose files, generates Updatecli manifests for detected update targets, and can optionally open pull requests through a supported SCM provider.
...
- detect Updatecli policy and manifest update opportunities
...
- `spec`: raw Updatecli autodiscovery crawler configuration

### Example Values

The following example updates discovered Updatecli targets with patch releases only and opens one pull request per detected update:

```yaml
name: "deps(updatecli): bump patch policies"
pipelineid: "updatecli/autodiscovery/patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/updatecli:0.11.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/updatecli:0.11.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/updatecli:0.11.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/updatecli" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/updatecli:0.11.0"
```
