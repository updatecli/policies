# Helmfile Autodiscovery Policy

This policy wraps Updatecli autodiscovery for Helmfile projects.
It scans repositories for `helmfile.yaml` files, generates Updatecli manifests for detected dependencies, and can optionally open pull requests through a supported SCM provider.
...
- detect Helm chart version updates in Helmfile configurations
...
The policy does not hardcode dependency rules. Instead, it passes your `spec` values directly to the Helmfile autodiscovery crawler so you can tune behavior with the same options supported by the Updatecli Helmfile autodiscovery plugin.
...
- `spec`: raw Helmfile autodiscovery crawler configuration

### Example Values

The following example updates Helmfile chart dependencies with patch releases only and opens one pull request per detected update:

```yaml
name: "deps(helmfile): bump patch dependencies"
pipelineid: "helmfile/autodiscovery/patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/helmfile:0.8.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/helmfile:0.8.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/helmfile:0.8.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/helmfile" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/helmfile:0.8.0"
```
...
1. Show the rendered manifest and verify that the `helmfile` crawler is present:
...
- Helmfile autodiscovery plugin docs: <https://www.updatecli.io/docs/plugins/autodiscovery/helmfile/>
