...
- detect argocd-managed dependency updates
...
The policy does not hardcode dependency rules. Instead, it passes your `spec` values directly to the argocd autodiscovery crawler so you can tune behavior with the same options supported by the Updatecli ArgoCD autodiscovery plugin.
...
- `spec`: raw argocd autodiscovery crawler configuration

### Example Values

The following example updates argocd-discovered dependencies with patch releases only and opens one pull request per detected update:

```yaml
name: "deps(argocd): bump patch dependencies"
pipelineid: "argocd/autodiscovery/patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/argocd:0.1.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/argocd:0.1.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/argocd:0.1.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/argocd" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/argocd:0.1.0"
```
...
1. Show the rendered manifest and verify that the `argocd` crawler is present:
...
- Argocd autodiscovery plugin docs: <https://www.updatecli.io/docs/plugins/autodiscovery/argocd/>
