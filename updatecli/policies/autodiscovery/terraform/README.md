# Terraform Autodiscovery Policy

This policy wraps Updatecli autodiscovery for Terraform projects.
It scans repositories for `*.tf` files, generates Updatecli manifests for detected dependencies, and can optionally open pull requests through a supported SCM provider.
...
- detect Terraform provider and module version updates
...
The policy does not hardcode dependency rules. Instead, it passes your `spec` values directly to the Terraform autodiscovery crawler so you can tune behavior with the same options supported by the Updatecli Terraform autodiscovery plugin.
...
- `spec`: raw Terraform autodiscovery crawler configuration

### Example Values

The following example updates Terraform provider/module dependencies with patch releases only and opens one pull request per detected update:

```yaml
name: "deps(terraform): bump patch dependencies"
pipelineid: "terraform/autodiscovery/patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/terraform:0.9.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/terraform:0.9.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/terraform:0.9.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/terraform" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/terraform:0.9.0"
```
...
1. Show the rendered manifest and verify that the `terraform` crawler is present:
...
- Terraform autodiscovery plugin docs: <https://www.updatecli.io/docs/plugins/autodiscovery/terraform/>
