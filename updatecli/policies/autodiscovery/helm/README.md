# Helm Autodiscovery Policy

This policy wraps Updatecli autodiscovery for Helm projects.
It scans repositories for Helm charts, generates Updatecli manifests for detected dependencies, and can optionally open pull requests through a supported SCM provider.
...
- detect Helm chart dependency updates
- detect container image updates referenced by Helm values and templates (depending on crawler settings)
...
The policy does not hardcode dependency rules. Instead, it passes your `spec` values directly to the Helm autodiscovery crawler so you can tune behavior with the same options supported by the Updatecli Helm autodiscovery plugin.
...
- `spec`: raw Helm autodiscovery crawler configuration

### Example Values

The following example updates Helm dependencies with patch releases only and opens one pull request per detected update:

```yaml
name: "deps(helm): bump patch dependencies"
pipelineid: "helm/autodiscovery/patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/helm:0.9.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/helm:0.9.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/helm:0.9.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/helm" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/helm:0.9.0"
```
...
1. Show the rendered manifest and verify that the `helm` crawler is present:
...
- Helm autodiscovery plugin docs: <https://www.updatecli.io/docs/plugins/autodiscovery/helm/>
