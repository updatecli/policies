# Pre-commit Autodiscovery Policy

This policy wraps Updatecli autodiscovery for Pre-commit projects.
It scans repositories for `.pre-commit-config.yaml` files, generates Updatecli manifests for detected dependencies, and can optionally open pull requests through a supported SCM provider.
...
- detect pre-commit hook version updates
...
The policy does not hardcode dependency rules. Instead, it passes your `spec` values directly to the Pre-commit autodiscovery crawler so you can tune behavior with the same options supported by the Updatecli Pre-commit autodiscovery plugin.
...
- `spec`: raw Pre-commit autodiscovery crawler configuration

### Example Values

The following example updates pre-commit hook dependencies with patch releases only and opens one pull request per detected update:

```yaml
name: "deps(precommit): bump patch dependencies"
pipelineid: "precommit/autodiscovery/patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/precommit:0.4.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/precommit:0.4.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/precommit:0.4.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/precommit" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/precommit:0.4.0"
```
...
1. Show the rendered manifest and verify that the `precommit` crawler is present:
...
- Pre-commit autodiscovery plugin docs: <https://www.updatecli.io/docs/plugins/autodiscovery/precommit/>
