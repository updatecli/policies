# GitHub Action Autodiscovery Policy

This policy wraps Updatecli autodiscovery for GitHub Action projects.
It scans repositories for GitHub Actions in workflow files, generates Updatecli manifests for detected actions, and can optionally open pull requests through a supported SCM provider.
...
- detect GitHub Action version updates
- detect GitHub Action updates from workflow files (`.github/workflows/*.yml`)
...
The policy does not hardcode dependency rules. Instead, it passes your `spec` values directly to the GitHub Action autodiscovery crawler so you can tune behavior with the same options supported by the Updatecli GitHub Action autodiscovery plugin.
...
- `spec`: raw GitHub Action autodiscovery crawler configuration

### Example Values

The following example updates GitHub Actions with minor and patch releases only and opens one pull request per detected update:

```yaml
name: "deps(github/actions): bump minor and patch"
pipelineid: "github/action/autodiscovery/minor-patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/githubaction:0.4.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/githubaction:0.4.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/githubaction:0.4.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/githubaction" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/githubaction:0.4.0"
```
...
1. Show the rendered manifest and verify that the `github/action` crawler is present:
...
- GitHub Action autodiscovery plugin docs: <https://www.updatecli.io/docs/plugins/autodiscovery/githubreaction/>
