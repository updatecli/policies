# Prow Autodiscovery Policy

This policy wraps Updatecli autodiscovery for Prow projects.
It scans repositories for Prow configuration files, generates Updatecli manifests for detected dependencies, and can optionally open pull requests through a supported SCM provider.
...
- detect Prow plugin and configuration updates
...
The policy does not hardcode dependency rules. Instead, it passes your `spec` values directly to the Prow autodiscovery crawler so you can tune behavior with the same options supported by the Updatecli Prow autodiscovery plugin.
...
- `spec`: raw Prow autodiscovery crawler configuration

### Example Values

The following example updates Prow dependencies with patch releases only and opens one pull request per detected update:

```yaml
name: "deps(prow): bump patch dependencies"
pipelineid: "prow/autodiscovery/patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/prow:0.6.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/prow:0.6.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/prow:0.6.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/prow" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/prow:0.6.0"
```
...
1. Show the rendered manifest and verify that the `prow` crawler is present:
...
- Prow autodiscovery plugin docs: <https://www.updatecli.io/docs/plugins/autodiscovery/prow/>
