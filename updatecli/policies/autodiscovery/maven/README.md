# Maven Autodiscovery Policy

This policy wraps Updatecli autodiscovery for Maven projects.
It scans repositories for `pom.xml` files, generates Updatecli manifests for detected dependencies, and can optionally open pull requests through a supported SCM provider.
...
- detect Maven dependency version updates
...
The policy does not hardcode dependency rules. Instead, it passes your `spec` values directly to the Maven autodiscovery crawler so you can tune behavior with the same options supported by the Updatecli Maven autodiscovery plugin.
...
- `spec`: raw Maven autodiscovery crawler configuration

### Example Values

The following example updates Maven dependencies with patch releases only and opens one pull request per detected update:

```yaml
name: "deps(maven): bump patch dependencies"
pipelineid: "maven/autodiscovery/patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/maven:0.9.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/maven:0.9.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/maven:0.9.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/maven" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/maven:0.9.0"
```
...
1. Show the rendered manifest and verify that the `maven` crawler is present:
...
- Maven autodiscovery plugin docs: <https://www.updatecli.io/docs/plugins/autodiscovery/maven/>
