# Terragrunt Autodiscovery Policy

This policy wraps Updatecli autodiscovery for Terragrunt projects.
It scans repositories for `terragrunt.hcl` files, generates Updatecli manifests for detected dependencies, and can optionally open pull requests through a supported SCM provider.
...
- detect Terraform module version updates in Terragrunt configurations
...
The policy does not hardcode dependency rules. Instead, it passes your `spec` values directly to the Terragrunt autodiscovery crawler so you can tune behavior with the same options supported by the Updatecli Terragrunt autodiscovery plugin.
...
- `spec`: raw Terragrunt autodiscovery crawler configuration

### Example Values

The following example updates Terragrunt module dependencies with patch releases only and opens one pull request per detected update:

```yaml
name: "deps(terragrunt): bump patch dependencies"
pipelineid: "terragrunt/autodiscovery/patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/terragrunt:0.6.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/terragrunt:0.6.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/terragrunt:0.6.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/terragrunt" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/terragrunt:0.6.0"
```
...
1. Show the rendered manifest and verify that the `terragrunt` crawler is present:
...
- Terragrunt autodiscovery plugin docs: <https://www.updatecli.io/docs/plugins/autodiscovery/terragrunt/>
