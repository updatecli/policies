...
This policy wraps Updatecli autodiscovery for NPM projects.
It scans repositories for `package.json` files, generates Updatecli manifests for detected dependencies, and can optionally open pull requests through a supported SCM provider.
...
- detect Node.js version updates
- detect NPM dependency updates from `package.json` and `package-lock.json`
...
- `spec`: raw NPM autodiscovery crawler configuration

### Example Values

The following example updates NPM dependencies with patch releases only and opens one pull request per detected update:

```yaml
name: "deps(npm): bump patch dependencies"
pipelineid: "npm/autodiscovery/patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/npm:0.14.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/npm:0.14.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/npm:0.14.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/npm" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/npm:0.14.0"
```
