# Kubernetes Autodiscovery Policy

This policy wraps Updatecli autodiscovery for Kubernetes projects.
It scans repositories for Kubernetes manifest files, generates Updatecli manifests for detected dependencies, and can optionally open pull requests through a supported SCM provider.
...
- detect container image updates in Kubernetes manifests
...
The policy does not hardcode dependency rules. Instead, it passes your `spec` values directly to the Kubernetes autodiscovery crawler so you can tune behavior with the same options supported by the Updatecli Kubernetes autodiscovery plugin.
...
- `spec`: raw Kubernetes autodiscovery crawler configuration

### Example Values

The following example updates Kubernetes container image dependencies with patch releases only and opens one pull request per detected update:

```yaml
name: "deps(kubernetes): bump patch dependencies"
pipelineid: "kubernetes/autodiscovery/patch"
groupby: individual
automerge: false
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/autodiscovery/kubernetes:0.9.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/autodiscovery/kubernetes:0.9.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/autodiscovery/kubernetes:0.9.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/autodiscovery/kubernetes" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/autodiscovery/kubernetes:0.9.0"
```
...
1. Show the rendered manifest and verify that the `kubernetes` crawler is present:
...
- Kubernetes autodiscovery plugin docs: <https://www.updatecli.io/docs/plugins/autodiscovery/kubernetes/>
