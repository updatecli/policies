# Node.js Netlify Policy

Automatically update the Node.js version referenced in `netlify.toml` to the latest Node.js release.
...
- updates the `NODE_VERSION = "..."` field in `netlify.toml`
- strips the leading `v` prefix from the version before writing (e.g. `v20.18.1` becomes `20.18.1`)

## Requirements

- `updatecli` CLI installed
- a GitHub token with read access to fetch release information
- a `netlify.toml` file in the repository with a `NODE_VERSION` variable
...
The following example updates the Node.js version in `netlify.toml` and opens a pull request:
...
targets:
  netlify:
    ...
    spec:
      file: netlify.toml
      key: build.environment.NODE_VERSION
```
...
```sh
updatecli manifest show --values values.yaml ghcr.io/updatecli/policies/nodejs/netlify:0.10.0
```

```sh
updatecli pipeline diff --values values.yaml ghcr.io/updatecli/policies/nodejs/netlify:0.10.0
```

```sh
updatecli pipeline apply --values values.yaml ghcr.io/updatecli/policies/nodejs/netlify:0.10.0
```
...
updatecli manifest push \
  ...
  --values values.yaml \
  --policy Policy.yaml \
  --tag "$OCI_REGISTRY/nodejs/netlify" \
  .
```
...
```sh
updatecli manifest show "$OCI_REGISTRY/nodejs/netlify:0.10.0"
```
...
2. Confirm that `netlify.toml` exists and contains a `NODE_VERSION` variable under `[build.environment]`.
